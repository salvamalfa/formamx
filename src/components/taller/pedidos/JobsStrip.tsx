...
export function JobsStrip({ jobs, onRetry }: { jobs: PrintJob[]; onRetry: (job: PrintJob) => void }) {
  return (
    <div class="flex flex-col gap-1.5 rounded-[var(--radius-s)] bg-[var(--hueso)] p-2.5">
      {jobs.map((job) => (
        <div key={job.id} class="flex flex-col gap-1">
          <div class="flex items-center gap-2.5">
            ...
            <div class="h-1.5 min-w-0 flex-1 overflow-hidden rounded-[var(--radius-pill)] bg-[var(--borde)]">
              <div
