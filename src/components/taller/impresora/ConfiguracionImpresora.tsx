    ...
    <Modal title="Configuración de Impresora" onClose={onClose} wide>
      <div class="flex gap-4">
        ...
        <div class="min-w-0 flex-1">
          {seccion === "calculadora" && <ConfigCostos />}
        </div>
      </div>
    </Modal>
  );
...
// dar clic en "Guardar". Antes cada campo se mandaba solo en su `blur`, así
// que abrir el popup y teclear ya cambiaba el precio de las piezas sin forma
// de arrepentirse — y como el input no se re-sincronizaba con la respuesta, un
...
const CAMPOS: Array<{
  ...
  {
    key: "costo_hora_mano_obra_mxn",
    label: "Mano de obra",
    unidad: "$/h",
    pesos: true,
  },
  {
    key: "minutos_mano_obra_default",
    label: "Mano de obra por pieza",
    unidad: "min",
  },
  {
    key: "precio_impresora_mxn",
...
function aBorrador(config: PricingConfig): Borrador {
  return Object.fromEntries(
    CAMPOS.map(({ key, pesos }) => [
      key,
      String(pesos ? config[key] / 100 : config[key]),
    ]),
  ) as Borrador;
}
...
function ConfigCostos() {
  ...
  const cambios = CAMPOS.reduce<Partial<Record<Clave, number>>>(
    (acc, { key }) => {
  ...
  const sucio = CAMPOS.some(({ key }) => {
    const valor = aGuardado(key, borrador[key]);
    return valor === null || valor !== config[key];
  });
  ...
  async function guardar() {
    ...
    // Se valida TODO antes de mandar nada: un campo malo no debe dejar a medias
    // los otros siete.
    const malos = CAMPOS.filter(
      ({ key }) => aGuardado(key, borrador[key]) === null,
    ).map(({ key }) => key);
    if (malos.length) {
      setInvalidos(malos);
  ...
  return (
    <div>
      <p class="m-0 mb-3 text-[12px] text-[var(--text-muted)]">
        Usados para el costo/precio automático de piezas de clientes (desglose
        "›" en Piezas de clientes).
      </p>
      ...
      <div class="mt-4 flex flex-wrap items-center justify-end gap-2 border-t border-[var(--border-soft)] pt-3">
        <span
          class="mr-auto text-[11px] text-[var(--text-faint)]"
          aria-live="polite"
        >
