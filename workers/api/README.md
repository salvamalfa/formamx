...
| `GET /api/admin/custom-prints` | `Bearer ADMIN_TOKEN` | Piezas de clientes (STL o proyecto 3MF); por defecto las no canceladas (`?status=`, `?limit=`). Cada pieza trae `formato` (`stl\|3mf`) y `progress_pct` del trabajo ligado cuando se está imprimiendo. |
| `POST /api/admin/custom-prints` | `Bearer ADMIN_TOKEN` | Sube un STL o un proyecto `.3mf` de Bambu Studio ("Guardar proyecto", no el plato exportado). El nombre va en `?filename=x.stl\|x.3mf` y el cuerpo es el archivo crudo (no multipart); `Content-Length` obligatorio (411 sin él) y máximo 100 MB. Un `.gcode.3mf` (plato ya rebanado, Fase 2 sin implementar) se rechaza con 400 `ya_rebanado`; otra extensión, 400 `nombre_invalido`. El binario se guarda en R2, D1 solo el metadato. |
| `POST /api/admin/custom-prints/:id/rebanar` | `Bearer ADMIN_TOKEN` | Encola el rebanado. Body `{"material","color_id","color_hex?","supports?","orient?"}` (`supports`: `auto\|no`, `orient`: `auto\|original`, ambos `auto` por defecto). Para una pieza `formato: "3mf"` se ignoran `supports`/`orient` del body (el proyecto ya los trae resueltos): se guardan `NULL`. Limpia los estimados anteriores; desde `subido\|listo\|fallido`. |
...
| `GET /api/agent/custom-prints/:id/stl` | `Bearer AGENT_TOKEN` | El STL o el proyecto 3MF crudo desde R2, para rebanarlo. |
