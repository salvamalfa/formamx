...
/** Las reglas de compatibilidad y el ajuste a móvil, sin el modo oscuro. */
function estilosBase(p: Paleta): string {
  return `
...
/**
 * Las declaraciones del modo oscuro, sin el `@media` que las envuelve.
 * `estilosCorreo` las mete dentro de un `@media (prefers-color-scheme: dark)`
 * para el correo real, donde el modo oscuro depende del tema del sistema.
 * `vista.ts` las reusa bajo una clase (`.forzar-oscuro`) para poder elegirlo
 * con un botón en vez de depender de eso.
 */
export function declaracionesOscuras(p: Paleta): string {
  return `
...
/**
 * Las reglas de estilo del correo, sin las etiquetas `<style>`. Vive aparte
 * de `envolver` porque la vista de revisión (`vista.ts`) también las necesita
 * sin el resto del documento.
 */
export function estilosCorreo(p: Paleta): string {
  return `${estilosBase(p)}
  ...
  @media (prefers-color-scheme: dark) {${declaracionesOscuras(p)}}
`;
}

/**
 * Solo la tabla del correo (cabecera, filas, pie), sin doctype/head/body.
 * `envolver` la usa para el documento completo que sube a Reach; `vista.ts`
 * la usa sola, porque un artifact no admite un documento HTML propio en su
 * página y necesita anclar comentarios a sus elementos — cosa que un iframe
 * con el documento completo no permite.
 */
export function tablaLienzo(edicion: Edicion, filas: string, p: Paleta): string {
  const numero = String(edicion.numero).padStart(3, '0');
  return `        <!--[if mso]><table role="presentation" width="${ANCHO}" cellpadding="0" cellspacing="0" border="0"><tr><td><![endif]-->
...
function cabecera(numero: string, p: Paleta): string {
  return `      <tr>
        <td class="px" style="padding:40px ${MARGEN}px 28px ${MARGEN}px;">
          ...
          <div class="meta" style="padding-top:16px;font-family:${FUENTE};font-size:12px;font-weight:700;letter-spacing:1.4px;text-transform:uppercase;color:${p.azul};">Bitácora &middot; Edición ${numero}</div>
        </td>
      </tr>`;
}
